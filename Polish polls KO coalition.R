...
# colours for plots
pcols <- c("KO"="orange", "PiS"="blue4", "PSL"="darkgreen", 
           "Kukiz'15"="black", "Wolność" = "deepskyblue", "Wiosna" = "purple", "Other"="gray50", 
           "MN"="yellow", "SLD"="red", "Razem"="purple")


## POOLED POLL MODEL
# read in, subset and adjust data
pollingdataPO <- read.csv('~/Desktop/Personal/Dropbox/Resources/Polish materials/Poll data/pooledpolls.csv')
pollingdataPO$KO <- pollingdataPO$PO + pollingdataPO$Nowoczesna
pollingdataPO$PO <- NULL
pollingdataPO$Nowoczesna <- NULL
pollingdata <- read.csv('~/Desktop/Personal/Dropbox/Resources/Polish materials/Poll data/pooledpollsKO.csv')
pollingdata <- rbind(pollingdataPO, pollingdata)
...
pollingdata$Wolność <- 100/((100-pollingdata$DK))*pollingdata$Wolność
...
pollingdata$DK <- NULL
pollingdata$n <- NULL
pollingdata$pdate <- julian(as.Date(pollingdata$date, "%d/%m/%Y"), origin=as.Date("2018-03-26"))
pollingdata$pdate <- as.Date(pollingdata$pdate, origin=as.Date("2018-03-26"))
pollingdata <- pollingdata[which(pollingdata$pdate > 0),]
pollingdata <- pollingdata[!is.na(pollingdata$pdate),]
pollingdata <- pollingdata[order(pollingdata$pdate),]
pollingdata$day <- as.integer(pollingdata$pdate)-as.integer(pollingdata$pdate)[1] + 1
...
PARTYNAMES <- c("KO","PiS","SLD","PSL","Wolność","Kukiz15","Razem", "Wiosna", "Other")
...
alpha[1] ~ dunif(250, 350) # KO
alpha[2] ~ dunif(350, 450) # PiS
alpha[3] ~ dunif(50, 100) # SLD
alpha[4] ~ dunif(50, 100) # PSL
alpha[5] ~ dunif(20, 40) # Wolność
alpha[6] ~ dunif(50, 100) # Kukiz15
alpha[7] ~ dunif(20, 40) # Razem
alpha[8] ~ dunif(50, 100) # Wiosna
alpha[9] ~ dunif(10, 30) # Other
...
results <- run.jags(model, monitor=c('walk','houseEffect','tightness'),data=data,n.chains=4,adapt=20000,
...
Wolnośćmean <- ppframe$Mean[grep('5]', ppframe$n)]*100
Wolnośćlow <- ppframe$Lower95[grep('5]', ppframe$n)]*100
Wolnośćhigh <- ppframe$Upper95[grep('5]', ppframe$n)]*100
Wolnośćest <- data.frame(Wolnośćmean, Wolnośćlow, Wolnośćhigh)
...
Razemmean <- ppframe$Mean[grep('7]', ppframe$n)]*100
Razemlow <- ppframe$Lower95[grep('7]', ppframe$n)]*100
Razemhigh <- ppframe$Upper95[grep('7]', ppframe$n)]*100
Razemest <- data.frame(Razemmean, Razemlow, Razemhigh)

Wiosnamean <- ppframe$Mean[grep('8]', ppframe$n)]*100
Wiosnalow <- ppframe$Lower95[grep('8]', ppframe$n)]*100
Wiosnahigh <- ppframe$Upper95[grep('8]', ppframe$n)]*100
Wiosnaest <- data.frame(Wiosnamean, Wiosnalow, Wiosnahigh)

Othermean <- ppframe$Mean[grep('9]', ppframe$n)]*100
Otherlow <- ppframe$Lower95[grep('9]', ppframe$n)]*100
Otherhigh <- ppframe$Upper95[grep('9]', ppframe$n)]*100
Otherest <- data.frame(Othermean, Otherlow, Otherhigh)

plotdata <- cbind(PiSest, KOest, PSLest, SLDest, Wolnośćest, Kukiz15est, Razemest, Wiosnaest, Otherest)
plotdata$date <- as.Date(c(1:length(KOmean)), origin=as.Date(tail(pollingdata$pdate, n=1)-length(KOmean)))

# LATEST FIGURES
# prepare data frame
party <- c("KO", "PiS", "SLD", "PSL", "Wolność", "Kukiz15", "Razem", "Wiosna", "Other")
alpha <- c(1,1,1,1,1,1,1,1,1)
percent <- round(c(mean(tail(KOest$KOmean, n=7)), mean(tail(PiSest$PiSmean, n=7)), 
                   mean(tail(SLDest$SLDmean, n=7)), mean(tail(PSLest$PSLmean, n=7)), 
                   mean(tail(Wolnośćest$Wolnośćmean, n=7)), mean(tail(Kukiz15est$Kukiz15mean, n=7)),
                   mean(tail(Razemest$Razemmean, n=7)), mean(tail(Wiosnaest$Wiosnamean, n=7)),
...
KO.PiS.diff <-pos[,"PiS"] - pos[,"KO"]
KO.PiS.diff.out <- sum(KO.PiS.diff > 0) / length(KO.PiS.diff)
KO.PiS.diff.out <- round(KO.PiS.diff.out, 2)
...
Wolność.thr <- pos[,"Wolność"] - 0.05
Wolność.thr.out <- sum(Wolność.thr > 0) / length(Wolność.thr)
Wolność.thr.out <- round(Wolność.thr.out, 2)
...
KOpct <- round(data1$KOcoef*means_pos[1,], digits=2)
...
Wolnośćpct <- round(data1$Wolnośćcoef*means_pos[5,], digits=2)
Kukiz15pct <- round(data1$Kukiz15coef*means_pos[6,], digits=2)
Razempct <- round(data1$Razemcoef*means_pos[7,], digits=2)
Wiosnapct <- round(data1$Wiosnacoef*means_pos[8,], digits=2)
...
poldHondt <- data.frame(KO=rep(1,42), Kukiz15=rep(1,42), MN=rep(1,42), PiS=rep(1,42), PSL=rep(1,42), 
                        Razem=rep(1,42), SLD=rep(1,42), Wiosna=rep(1,42), Wolność=rep(1,42))
for( i in 1 : 42 ) {
  poldHondt[i, ] <- dHondt(c("KO", "Kukiz15", "MN", "PiS", "PSL", "Razem", "SLD", "Wiosna", "Wolność"), 
                           c(KOest[i], Kukiz15est[i], MNest[i], PiSest[i], PSLest[i], 
                             Razemest[i], SLDest[i], Wiosnaest[i], Wolnośćest[i]), data1$magnitude[i])
}
...
frame$in2015[frame$Party=="Wolność"] <- 0
...
frame$Party <- factor(frame$Party, levels=c("KO", "PiS", "SLD", "PSL", "MN", "Wolność", "Kukiz15", "Wiosna", "Razem"))
...
ggplot(data=posfrmelt, aes(variable, value)) + 
  ...
  annotate(geom = "text", label=paste(round(100*mean(posfrmelt$value[posfrmelt$variable=="KO"]),0)), 
           x="KO", y=mean(posfrmelt$value[posfrmelt$variable=="KO"]), size=3.75, hjust = "center", vjust=-3) +
  annotate(geom = "text", label=paste(round(100*mean(posfrmelt$value[posfrmelt$variable=="PiS"]),0)), 
           x="PiS", y=mean(posfrmelt$value[posfrmelt$variable=="PiS"]), size=3.75, hjust = "center", vjust=-3) +
  annotate(geom = "text", label=paste(round(100*mean(posfrmelt$value[posfrmelt$variable=="SLD"]),0)), 
           x="SLD", y=mean(posfrmelt$value[posfrmelt$variable=="SLD"]), size=3.75, hjust = "center", vjust=-3) +
  annotate(geom = "text", label=paste(round(100*mean(posfrmelt$value[posfrmelt$variable=="Kukiz'15"]),0)), 
           x="Kukiz'15", y=mean(posfrmelt$value[posfrmelt$variable=="Kukiz'15"]), size=3.75, hjust = "center", vjust=-3) +
  annotate(geom = "text", label=paste(round(100*mean(posfrmelt$value[posfrmelt$variable=="PSL"]),0)), 
           x="PSL", y=mean(posfrmelt$value[posfrmelt$variable=="PSL"]), size=3.75, hjust = "center", vjust=-3) +
  annotate(geom = "text", label=paste(round(100*mean(posfrmelt$value[posfrmelt$variable=="Wolność"]),0)), 
           x="Wolność", y=mean(posfrmelt$value[posfrmelt$variable=="Wolność"]), size=3.75, hjust = "center", vjust=-3) +
  annotate(geom = "text", label=paste(round(100*mean(posfrmelt$value[posfrmelt$variable=="Razem"]),0)), 
           x="Razem", y=mean(posfrmelt$value[posfrmelt$variable=="Razem"]), size=3.75, hjust = "center", vjust=-3) +
  annotate(geom = "text", label=paste(round(100*mean(posfrmelt$value[posfrmelt$variable=="Wiosna"]),0)), 
           x="Wiosna", y=mean(posfrmelt$value[posfrmelt$variable=="Wiosna"]), size=3.75, hjust = "center", vjust=-3) +
  annotate(geom = "text", label=paste(round(100*mean(posfrmelt$value[posfrmelt$variable=="Other"]),0)), 
           x="Other", y=mean(posfrmelt$value[posfrmelt$variable=="Other"]), size=3.75, hjust = "center", vjust=-3) +
  annotate(geom = "text", label=paste("Pr(PiS > 50%)  = ", PiS.50.out), x=5.5, y=0.25, size=3.5, adj=c(0)) +
  annotate(geom = "text", label=paste("Pr(PiS > KO)  = ", KO.PiS.diff.out), x=5, y=0.25, size=3.5, adj=c(0)) +
  ...
  annotate(geom = "text", label=paste("Pr(PSL > 5%)  = ", PSL.thr.out), x=3, y=0.25, size=3.5, adj=c(0)) +
  annotate(geom = "text", label=paste("Pr(Wolność > 5%)  = ", Wolność.thr.out), x=2.5, y=0.25, size=3.5, adj=c(0)) +  
  annotate(geom = "text", label=paste("Pr(Razem > 5%)  = ", Razem.thr.out), x=2, y=0.25, size=3.5, adj=c(0)) +
...
ggsave('Latest_figures_KO.png')

# plot trends
datl <- melt(plotdata, measure.vars=c("KOmean","PiSmean","SLDmean","PSLmean","Wolnośćmean", "Wiosnamean", "Kukiz15mean", "Razemmean", "Othermean"))
levels(datl$variable) <- c("KO","PiS", "SLD","PSL","Wolność", "Wiosna", "Kukiz'15", "Razem", "Other")
datl$variable <- factor(datl$variable, levels = c("KO", "PiS", "SLD", "PSL", "Wolność", "Wiosna", "Kukiz'15", "Razem", "Other"))

pdatl <- melt(pollingdata, measure.vars=c("KO","PiS","SLD","PSL","Wolność", "Wiosna", "Kukiz15", "Razem", "Other"))
levels(pdatl$variable) <- c("KO","PiS", "SLD","PSL","Wolność", "Wiosna", "Kukiz'15", "Razem", "Other")
pdatl$variable <- factor(pdatl$variable, levels = c("KO", "PiS", "SLD", "PSL", "Wolność", "Wiosna", "Kukiz'15", "Razem", "Other"))

datl_POPiS <- datl[!(datl$variable %in% c("Kukiz'15", "PSL", "SLD", "Wolność", "Wiosna", "Other", "Razem")),]
ggplot(datl_POPiS, aes(x=date, y=value, colour=factor(variable))) + geom_line() + 
  ...
  scale_colour_manual(name="", values=pcols, breaks=c("PiS", "KO")) + 
  guides(color=guide_legend(override.aes=list(fill=NA)))
ggsave('KOPiS_trends_KO.png')

datl_other <- datl[!(datl$variable %in% c("KO", "PiS", "Wolność", "Razem", "Other")), ]
ggplot(datl_other, aes(x=date, y=value, colour=factor(variable))) + geom_line() + 
  ...
  geom_ribbon(data=subset(datl, variable=="Wiosna"),aes(ymin=Wiosnalow, ymax=Wiosnahigh), colour=NA, fill="purple", alpha=0.3) +
  ...
  geom_point(data=pollingdata, aes(x=as.Date(pollingdata$date, "%d/%m/%Y"), y=Wiosna), col="purple", size=1.5) +
  theme(panel.background = element_rect(colour="white"),  axis.text.x = element_text(size=10), 
        axis.text.y = element_text(size=10), axis.title.x = element_blank(), axis.title.y = element_blank(),
        plot.margin = unit(c(1,3,1,1), "lines"), strip.text.x = element_text(size = 10))+
  background_grid(major = "xy", minor = "none") +
  scale_x_date(labels=date_format("%d.%m.%y"))+
  scale_colour_manual(name="", values=pcols, breaks=c("Kukiz'15", "PSL", "SLD", "Wiosna"), 
                      labels=c("Kukiz'15", "PSL", "SLD", "Wiosna")) + 
  guides(color=guide_legend(override.aes=list(fill=NA)))
ggsave('Other_trends_KO.png')

datl_minor <- datl[!(datl$variable %in% c("KO", "PiS", "Kukiz'15", "PSL", "SLD", "Wiosna")), ]
ggplot(datl_minor, aes(x=date, y=value, colour=factor(variable))) + geom_line() + 
  geom_abline(intercept=5, slope=0, colour="black", linetype=2) +
  geom_ribbon(data=subset(datl, variable=="Wolność"),aes(ymin=Wolnośćlow, ymax=Wolnośćhigh), colour=NA, fill="deepskyblue", alpha=0.3) +
  geom_ribbon(data=subset(datl, variable=="Razem"),aes(ymin=Razemlow, ymax=Razemhigh), colour=NA, fill="purple", alpha=0.3) +
  geom_ribbon(data=subset(datl, variable=="Other"),aes(ymin=Otherlow, ymax=Otherhigh), colour=NA, fill="gray50", alpha=0.3) +
  geom_point(data=pollingdata, aes(x=as.Date(pollingdata$date, "%d/%m/%Y"), y=Wolność), col="deepskyblue", size=1.5) +
  geom_point(data=pollingdata, aes(x=as.Date(pollingdata$date, "%d/%m/%Y"), y=Razem), col="purple", size=1.5) +
  ...
  background_grid(major = "xy", minor = "none") +
  scale_x_date(labels=date_format("%d.%m.%y"))+
  scale_colour_manual(name="", values=pcols, breaks=c("Wolność", "Razem", "Other"), 
                      labels=c("Wolność", "Razem", "Other")) + 
  guides(color=guide_legend(override.aes=list(fill=NA)))
ggsave('Minor_trends_KO.png')
...
ggplot(datl, aes(x=date, y=value, colour=factor(variable))) + geom_line() +
  ...
  geom_point(data=subset(pdatl, variable=="KO"), aes(x=as.Date(date, "%d/%m/%Y"), y=value), col="orange", size=1.5) +
  ...
  geom_point(data=subset(pdatl, variable=="Wiosna"), aes(x=as.Date(date, "%d/%m/%Y"), y=value), col="purple", size=1.5) +
  geom_point(data=subset(pdatl, variable=="SLD"), aes(x=as.Date(date, "%d/%m/%Y"), y=value), col="red", size=1.5) +
  geom_point(data=subset(pdatl, variable=="Wolność"), aes(x=as.Date(date, "%d/%m/%Y"), y=value), col="deepskyblue", size=1.5) +
  geom_point(data=subset(pdatl, variable=="Razem"), aes(x=as.Date(date, "%d/%m/%Y"), y=value), col="purple", size=1.5) +
...
ggsave('Overall_trends_KO.png')

#current seat share without district weighting
levels(frame$Party)[levels(frame$Party)=="Kukiz15"] <- "Kukiz'15"
levels(frame$Party)[levels(frame$Party)=="KO"] <- "KO"
ggplot(data=frame, mapping=aes(x=Party, y=Unweighted, fill=Party)) +
  ...
  scale_fill_manual(name="Party", values = pcols, 
                    breaks=c("KO", "PiS", "SLD", "PSL", "Wiosna", "Wolność", "MN", "Kukiz'15", "Razem"), 
                    labels=c("KO", "PiS", "SLD", "PSL", "Wiosna", "Wolność", "MN", "Kukiz'15", "Razem"))+
...
ggsave('Seats_unweighted_KO.png')
...
ggplot(data=frame, mapping=aes(x=Party, y=Weighted, fill=Party)) +
  ...
  scale_y_continuous('Number of seats', limits=c(0,320), breaks=c(0, 50, 100, 150, 200, 231, 276, 307)) +
  scale_fill_manual(name="Party", values = pcols, 
                    breaks=c("KO", "PiS", "SLD", "PSL", "Wiosna", "Wolność", "MN", "Kukiz'15", "Nowoczesna", "Razem"), 
                    labels=c("KO", "PiS", "SLD", "PSL", "Wiosna", "Wolność", "MN", "Kukiz'15", "Nowoczesna", "Razem"))+
  theme(plot.margin = unit(c(1,1,1,1), "lines"),
        legend.position="none", axis.text.x=element_text(colour="black", size=10), 
        axis.text.y= element_text(colour="black", size=10), axis.title.x = element_blank()) +
  geom_label(aes(x=2, y=231), label="Legislative majority", size=3, adj=c(0), fill="grey95") +
...
ggsave('Seats_weighted_KO.png')

# seats table
seats <- cbind(poldHondt, data1)
row.names(seats) <- data1$name
keep <- c("PiS","KO","Kukiz15","SLD","PSL","Wolność", "Wiosna", "Razem")
seats <- seats[keep]
...
ggsave('PiS_seats_KO.png', width=7, height=7)
...
ggplot(plotdata) + 
  ...
  theme_void() + theme(aspect.ratio=1, legend.position="none")
ggsave('KO_seats_KO.png', width=7, height=7)
...
ggplot(plotdata) + 
  ...
  theme_void() + theme(aspect.ratio=1, legend.position="none")
ggsave('PSL_seats_KO.png', width=7, height=7)
...
ggplot(plotdata) + 
  ...
  theme_void() + theme(aspect.ratio=1, legend.position="none")
ggsave('SLD_seats_KO.png', width=7, height=7)
...
ggplot(plotdata) + 
  ...
  theme_void() + theme(aspect.ratio=1, legend.position="none")
ggsave('Kukiz15_seats_KO.png', width=7, height=7)
...
ggplot(plotdata) + 
  ...
  geom_path(color="black") +
  theme(aspect.ratio=1) +
  scale_fill_gradient(name="Wiosna", limits=c(min=0, max=20), low = "white", high = "blue4", guide="colorbar") +
  geom_label(seats, mapping = aes(x=label_point_x, y=label_point_y, group=Wiosna, label=Wiosna), fill="white") +
  theme_void() + theme(aspect.ratio=1, legend.position="none")
ggsave('Wiosna_seats_KO.png', width=7, height=7)
...
ggplot(plotdata) + 
  ...
  theme_void() + theme(aspect.ratio=1, legend.position="none")
ggsave('PiSKO_seats_KO.png', width=7, height=7)
...
KOspread <- ggplot(plotdata) + 
  aes(long,lat,group=group,fill=as.integer(KO)) + 
  geom_polygon() +
  geom_path(color="black") +
  scale_fill_gradient(name="", limits=c(min=0, max=max(seats$KO)), breaks=c(0, max(seats$KO)),
...
Wiosnaspread<- ggplot(plotdata) + 
  ...
  scale_fill_gradient(name="", limits=c(min=0, max=max(seats$Wiosna)), breaks=c(0, max(seats$Wiosna)),
                      low = "white", high = "purple", guide="colorbar") +
  theme_void() + theme(aspect.ratio=1, legend.key.size = unit(0.7, "line")) +
  ggtitle("Wiosna") +
  theme(plot.title = element_text(size=10, hjust = 0.5, face="bold")) +
  theme(plot.margin=grid::unit(c(5,0,0,0), "mm"))

seat_spreads <- plot_grid(PiSspread, KOspread, Wiosnaspread, Kukiz15spread, PSLspread, SLDspread)
ggsave('Seat_spreads_KO.png', seat_spreads)
