...
polls <-
  ...
  gather(candidate, percent, Duda, Trzaskowski, Biedroń, `Kosiniak-Kamysz`, Bosak, Hołownia, Other, na.rm = FALSE, convert = FALSE) %>%
...
p_trends$candidate <- fct_reorder(p_trends$candidate, p_trends$delta, .fun=median, .desc=TRUE)
polls$candidate <- fct_reorder(polls$candidate, polls$percent, .fun=median, .desc=TRUE)

p_trends_plot_r1 <- ggplot(p_trends) +
...
p_trends_facet_plot_r1 <- ggplot(p_trends) +
  stat_lineribbon(aes(x = time, y = delta*100, fill=candidate, color=candidate), .width=c(0.5, 0.66, 0.95), alpha=1/4, show.legend = FALSE) +
  geom_point(data = polls, aes(x = midDate, y = percent, color=candidate), alpha = 1, size = 2, show.legend = FALSE) +
...
p_latest_data$candidate <- fct_reorder(p_latest_data$candidate, p_latest_data$delta, .fun=median, .desc=TRUE)

p_latest_plot_r1 <- ggplot(p_latest_data) +
  ...
  annotate(geom = "text", label=paste("Probability of Duda winning:", 
           ...
           y=6.8, x=median(p_latest_data$delta[p_latest_data$candidate=="Duda"])*100, 
  ...
  annotate(geom = "text", label=paste("Probability of Trzaskowski winning:", 
           ...
           y=5.8, x=median(p_latest_data$delta[p_latest_data$candidate=="Trzaskowski"])*100, 
           size=3.75, family="Roboto Condensed") +
  ...
  annotate(geom = "text", label=paste(round(100*median(p_latest_data$delta[p_latest_data$candidate=="Trzaskowski"]),0)), 
           ...
           family="Roboto Condensed", color="white") +
  annotate(geom = "text", label=paste(round(100*median(p_latest_data$delta[p_latest_data$candidate=="Hołownia"]),0)), 
           y="Hołownia", x=mean(round(100*median(p_latest_data$delta[p_latest_data$candidate=="Hołownia"]),0)), size=4, hjust = "center", vjust=-1, 
           family="Roboto Condensed", color="white") +
  annotate(geom = "text", label=paste(round(100*median(p_latest_data$delta[p_latest_data$candidate=="Bosak"]),0)), 
           y="Bosak", x=mean(round(100*median(p_latest_data$delta[p_latest_data$candidate=="Bosak"]),0)), size=4, hjust = "center", vjust=-1, 
           family="Roboto Condensed", color="white") +
  annotate(geom = "text", label=paste(round(100*median(p_latest_data$delta[p_latest_data$candidate=="Kosiniak-Kamysz"]),0)), 
           y="Kosiniak-Kamysz", x=mean(round(100*median(p_latest_data$delta[p_latest_data$candidate=="Kosiniak-Kamysz"]),0)), size=4, hjust = "center", vjust=-1, 
           family="Roboto Condensed", color="white") +
  annotate(geom = "text", label=paste(round(100*median(p_latest_data$delta[p_latest_data$candidate=="Biedroń"]),0)), 
           y="Biedroń", x=mean(round(100*median(p_latest_data$delta[p_latest_data$candidate=="Biedroń"]),0)), size=4, hjust = "center", vjust=-1, 
           family="Roboto Condensed", color="white") +
  annotate(geom = "text", label=paste(round(100*median(p_latest_data$delta[p_latest_data$candidate=="Other"]),0)), 
           y="Other", x=mean(round(100*median(p_latest_data$delta[p_latest_data$candidate=="Other"]),0)), size=4, hjust = "center", vjust=-1, 
           family="Roboto Condensed", color="white") +
  labs(x = "% of vote", y="",
       subtitle = "",
       color="",
  ...
  theme_minimal() +
  theme_ipsum_rc()
ggsave(p_latest_plot_r1, file = "p_latest_plot_r1.png", 
