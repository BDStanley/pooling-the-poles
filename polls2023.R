...
cols <- c("PiS"="blue", "KO"="orange", "Trzecia Droga" = "darkgreen", "Lewica"="red", "Konfederacja" = "midnightblue", "MN" = "yellow")
...
poldHondt <- data.frame(KO=rep(1,41000), Konfederacja=rep(1,41000), Lewica=rep(1,41000), MN=rep(1,41000), PiS=rep(1,41000), `Polska 2050`=rep(1,41000))

for(i in 1:41000) { 
  poldHondt[i,] <- giveseats(v = c(consts$KO[i], consts$Konfederacja[i], consts$Lewica[i],consts$MN[i], consts$PiS[i], consts$`Polska 2050`[i]), 
                             ns = consts$magnitude[i], method="dh", thresh=0)$seats
}

poldHondt <- cbind(poldHondt, consts$okreg, consts$.draw)

colnames(poldHondt) <- c("KO", "Konfederacja", "Lewica", "MN", "PiS", "Polska 2050", "okreg", "draw")
...
poldHondt <- poldHondt %>%
  pivot_longer(., cols=c("KO", "Konfederacja", "Lewica", "MN", "PiS", "Polska 2050"), names_to="party", values_to="seats")
...
#####Plots#####
plot_seats_p2050_psl <- ggplot(data=frame, mapping=aes(x=party, y=y, fill=party)) +
...
plot_latest_p2050_psl <- plotdraws %>%
  ...
  expand_limits(x = 0) +
  labs(caption="Ben Stanley (@BDStanley; benstanley.pl).", x="", title="Poparcie dla partii politycznych", subtitle="(95%-owy przedział wiarygodności)", color="") +
  theme_plots()
ggsave(plot_latest_p2050_psl, file = "plot_latest_p2050_psl.png",
